...
for direction in snakeDirectionsB:
    i = 1
    steps = 0
    if direction.startswith("U"):
        steps = int(direction.replace("U", ""))
        while i <= steps:
            nextPoint = [highResolutionTrailB[-1][0], highResolutionTrailB[-1][1]]
            nextPoint[1] = nextPoint[1] + 1
            highResolutionTrailB.append(nextPoint)
            i += 1
...
j = 0
for point in highResolutionTrailA:
    if j % 1000 == 0:
        print(str(round(j / stepsInTrailA * 100)) + "% complete - Manhattan Distance: " + str(closestPointManhattanDistance) + " found at point " + str(closestPoint) + " with shortest trail intersection of " + str(shortestTrailIntersection), end='\r')
    if point in highResolutionTrailB:
        updatePointOfInterest(point)
        updateShortestTrailIntersection(point)
    j += 1
